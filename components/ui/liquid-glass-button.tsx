...
const liquidbuttonVariants = cva(
  "inline-flex items-center transition-colors justify-center cursor-pointer gap-2 whitespace-nowrap text-sm font-medium transition-[color,box-shadow] disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
  {
    variants: {
      variant: {
        default: "bg-transparent hover:scale-105 transition text-primary",
      },
      size: {
        default: "h-9 px-4 py-2",
        sm: "h-8 px-4",
    ...
    defaultVariants: {
      variant: "default",
      size: "xxl",
      radius: "full", // 👈 default rounded full
    },
  }
);
  ...
  return (
    <>
      <Comp
        ...
        className={cn(
          "relative",
          liquidbuttonVariants({ variant, size, radius, className })
        )}
        {...props}
      >
...
> = {
  ...
  orange: {
    // Bright-but-balanced metallic orange ring
    outer: "bg-gradient-to-b from-[#FF6A00] to-[#FFB067]",

    // Inner glow: soft bright → warm mid → creamy highlight
    inner: "bg-gradient-to-b from-[#FFE4C7] via-[#FF7A17] to-[#FFCDA1]",

    // Main button body: deeper orange → rich amber
    button: "bg-gradient-to-b from-[#FFB876] to-[#E5943D]",

    // Clean UI white text
    textColor: "text-white",

    // Slight warm shadow for depth
    textShadow: "[text-shadow:_0_-1px_0_rgb(160_70_10_/_100%)]",
  },
...
const metalButtonVariants = (
  ...
  return {
    ...
    buttonStyle: {
      transform: isPressed ? "scale(0.88) skewX(1deg) skewY(1deg)" : "scale(1)",

      transition: "transform 300ms cubic-bezier(0.25, 1.6, 0.35, 1)", // liquid rebound
